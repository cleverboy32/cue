...
import { Watcher, observe } from './observerble.js'
...
function genarateDom (root, rootEle) {
    ...
    nodes.map((item) => {
        ...
        for (let attr of item.attrs) {
            ...
            if (event) { //是事件
                ele.addEventListener(event[1], () => {
                    options.methods[attr.value].call(data);
                }, true);
            } else {
        ...
        // 替换变量
        let text = item.text.replace(/{{(.*)}}/g, ($1, $2) => {
            return ' ' + data[$2];
        })
